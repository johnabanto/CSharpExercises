...
namespace CSharpExercises
{
    public static class Exercises
    {
        ...
        {
            double sum = 0;
            // foreach will look at all the int numbers in the array.
        ...
        public static string GetMilesPerHour(double distance, int hours, int minutes, int seconds)
        {
            ...
            int secondsPerHour = minutesPerHour * 60;
        ...
        public static int GetOccurences(string toCheck, string checkFor)
        {
            string[] check = { checkFor };
            return toCheck.Split(check, StringSplitOptions.RemoveEmptyEntries).Length - 1;
            // split the string using the substring and use the length for the array.
        }
        ...
        public static double MortgageCalculator(double loanBalance, double interestRate, int loanYears, int paymentPeriod)
        {
            double mortgagePayment = 0;
            double x = 0;
            double yearlyInterestRate = interestRate / 100;
            double monthlyInterestRate = yearlyInterestRate / 12;
            int monthsOfPayment = loanYears * paymentPeriod;

            x = Math.Pow((1 + monthlyInterestRate), monthsOfPayment);

            mortgagePayment = (loanBalance * yearlyInterestRate * x) / (12 * (x - 1));
            //Console.WriteLine(mortgagePayment);
